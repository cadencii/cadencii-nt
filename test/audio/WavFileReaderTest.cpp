...
TEST(WavFileReaderTest, stereo16_44100)
{
    WavFileReader reader( TestUtil::getFixtureRootPath() + "/fixture/audio/WavFileReaderTest/44100stereo16.wav" );

    EXPECT_EQ( 44100, reader.getSampleRate() );
    EXPECT_EQ( (uint32_t)44100, reader.getTotalSamples() );
    ...
    for( int i = 0; i < count; i++ ){
        ...
        for( int j = 0; j < 25; j++ ){
            EXPECT_FLOAT_EQ( fixture[j], actualLeft[j] );
            EXPECT_FLOAT_EQ( -fixture[j], actualRight[j] );
        }
    }
    ...
    reader.pull( actualLeft, actualRight, 25 );
    for( int i = 0; i < 25; i++ ){
        EXPECT_FLOAT_EQ( 0.0, actualLeft[i] );
        EXPECT_FLOAT_EQ( 0.0, actualRight[i] );
    }
}

TEST(WavFileReaderTest, monoral16_44100)
{
...
TEST(WavFileReaderTest, stereo8_44100)
{
...
TEST(WavFileReaderTest, monoral8_44100)
{
